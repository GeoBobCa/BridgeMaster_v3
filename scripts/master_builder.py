...
# --- 1. THE CLEAN BASE CONTENT (Dealer + 1H + 1S + 1NT) ---
BASE_CONTENT = """Dealer:
  - bid: "Pass"
    type: "Opening"
    convention: "Natural"
...
SYSTEM_PROMPT = """
### Audrey Grant Modern Standard System Definition
1. Hand Evaluation:
   - Open 1NT: 15-17 Balanced.
   - Open 2NT: 20-21 Balanced.
   - Open 2C: 22+ points.
2. Priority of Opening:
   - 5-card Major (1H/1S).
   - Better Minor (1C/1D). 3-3 -> 1C. 4-4 -> 1D.
3. Minor Suit Responses (To 1C/1D):
   - New Suit: Up the line (4+ cards).
   - Raises: 1C-2C usually requires 5+ Clubs. 6-10 HCP.
   - No Major: 1NT (6-9), 2NT (11-12), 3NT (13-15).
"""

def get_yaml_for_key(key_name):
    print(f"🧠 Asking Gemini for '{key_name}' logic...")
    prompt = f"""
    ACT AS: Bridge System Architect.
    TASK: Generate the YAML nodes for the children of the auction: "{key_name}".
...
def main():
    ...
    TARGET_FILE = PROJECT_ROOT / "systems" / "bidding_tree.yaml"
    
    print("--- STEP 1: RESTORING BASE DATABASE ---")
    with open(TARGET_FILE, "w", encoding="utf-8") as f:
        f.write(BASE_CONTENT)
    print("✅ Base system restored.")
    
    print("\n--- STEP 2: GENERATING 1 DIAMOND ---")
    d_block = get_yaml_for_key("1D")
    
    print("\n--- STEP 3: GENERATING 1 CLUB ---")
    c_block = get_yaml_for_key("1C")
    ...
    print(f"\n🎉 DONE! Full system built at: {TARGET_FILE}")
    print("Run 'python tools/tree_viewer.py' to see it.")
